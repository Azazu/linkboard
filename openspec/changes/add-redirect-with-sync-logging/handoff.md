...
## Next step
`/opsx:apply add-redirect-with-sync-logging` — implement per tasks.md (block 1 click write model → 2 redirect incl. the security.yaml firewall pattern → 3 web tests → 4 docs → 5 wrap-up), commit per block with every guard's failing input in the commit body, `make check`, then the user pushes the branch for a green run, then `scripts/gate-run.sh add-redirect-with-sync-logging 2 full`. Security-sensitive parts to flag in commits and here: firewall pattern, hot-path header handling, Location composition, IP hashing.
