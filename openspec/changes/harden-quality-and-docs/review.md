...
**Reviewer:** codex
**Date:** 2026-09-16
**Reviewed-Commit:** 448b8cd09a349d576c59fc663b3bc9ea5c829c38
**Verdict:** changes-requested

### Findings
| # | Resolution |
|---|------------|
| 1 | changes-requested — `scripts/test-jwt-passphrase.php:25–34` fixes comment parsing but still does not resolve the bootstrap's effective environment. The bootstrap publishes every parsed variable to `$_ENV`, `$_SERVER` and `putenv()` after each file; the helper only saves the JWT value in a local variable and starts a fresh parser for the next file. For a valid `.env.test.local` containing `JWT_PASSPHRASE="${JWT_PASSPHRASE}-tail"`, with an initially empty process value, the helper therefore resolves `-tail`, while the bootstrap resolves the committed test value followed by `-tail`. This follows directly from the installed Dotenv variable-resolution order: its per-parse values are reset, and references fall back to the process environment. The helper also omits the bootstrap's initial `bootEnv()` setup. Match the bootstrap's environment initialization and publication between files, then verify cross-file expansion, generation, preservation and authentication. The same-parser claims in the script, command reference and handoff currently overstate the fix. |
| 2 | changes-requested — The new script supplies the missing authentication, nine-report loop, response checks and aggregation, but the published recipe remains non-executable in its stated sequence. `docs/how-to/benchmarks.md:34–53` switches the PHP container to `APP_ENV=prod` and keeps it there through section 3; line 90 then runs `app:demo:seed` without an environment override. `DemoSeedCommand.php:57–60` explicitly returns failure in prod, so section 2 cannot establish its million-row fixture. Make the seeding and measurement environments explicit and execute the corrected sequence. Also, `scripts/report-benchmark.sh:47–50` extracts the last `id` from a compact JSON collection: its greedy sed expression returns `least-clicked` for `[{"id":"most-clicked","clickCount":1000},{"id":"least-clicked","clickCount":1}]`, despite requesting descending order. Parse the collection structurally and select its first item so the measured link matches the documented fixture; rerun and reconcile the table, README and task evidence. These are remaining reproducibility defects in the named finding, not unrelated findings. |

### Validation

- Reviewed only the diff from `c69f769782ee9688d5092ae4e3a5b74d25bef826` to the Reviewed-Commit and collateral material relevant to the two source-round major findings. Both were marked `fixed`; no source-round finding remained `open`. No unrelated minor findings were introduced.
- Verified branch `change/harden-quality-and-docs`, HEAD equal to the requested target, and an initially clean working tree. Read the repository protocol, OpenSpec configuration, change proposal and relevant task/handoff evidence.
- Traced passphrase resolution through `tests/bootstrap.php`, the new helper, the Makefile, and installed Symfony Dotenv `parse()` / `resolveVariables()`. The cross-file case above is a source-derived counterexample, not a claimed PHP execution.
