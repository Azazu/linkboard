...
- **And the same divergence one layer down, in key generation.** `make jwt-keys` runs `lexik:jwt:generate-keypair --env=test` inside the container, where `.env`'s empty `JWT_PASSPHRASE` wins. Measured rather than assumed (Gate 1 round 2, finding 1): the key it writes is **encrypted with an empty passphrase** — `-----BEGIN ENCRYPTED PRIVATE KEY-----`, opens with `pass:`, and *refuses* the passphrase `.env.test` declares. A fresh `make init` therefore produces a test keypair the suite cannot use; CI is unaffected, because it runs the same target natively where `.env.test` applies. The target learns to generate the test key with the effective test passphrase, and to replace a key that does not match it instead of skipping over one.
