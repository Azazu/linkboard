...
#### Scenario: JSON objects where arrays are required
- **WHEN** a user posts, and then patches an existing link with, documents whose `rules` is `{"0":{"match":{"country":["DE"]},"target":"https://example.com/"}}`, whose first rule has `"country": {"0":"DE"}`, and whose `variants` is `{"0":{"name":"A","weight":50,"target":"https://example.com/a"},"1":{"name":"B","weight":50,"target":"https://example.com/b"}}`
- **THEN** each response status is 422 with a single violation at, respectively, `rules[rules]`, `rules[rules][0][match][country]` and `rules[variants]`, and after each rejected patch the stored `rules` is unchanged
