...
4. **Production deep health probe for monitors** (health-check spec, FR-KEY-2 ∩ §2.10): `GET /health?deep=1` in `prod` runs the probe when the request carries a valid, unexpired, unrevoked API key of an unblocked **admin** — looked up by hash on a time-bounded connection of its own (like the probe's checks), failing closed: a database that is down or stalled yields the same 404 within the budget, never a hang and never an unauthorized probe; anything else keeps today's 404. Sessions and JWTs never authorize it.
...
- Modified: `config/packages/security.yaml` (`access_token` on the `api` firewall), `config/packages/framework.yaml` (`api_identity` limiter), `.env` (`RATE_LIMIT_API_PER_KEY=600`), `src/Shared/Health/HealthController.php` and `HealthProbe.php` (admin-key branch; the bounded connection factory extracted), `.github/workflows` (`make migrate EXEC=` so the prod-kernel tests find the schema in the `app` database), `docs/how-to/local-development.md`, `docs/explanation/requirements.md` (FR-KEY-1…4), `docs/reference/commands.md` if a console helper is added (none planned), `openspec/ROADMAP.md` (row 10 removed at archive time).
