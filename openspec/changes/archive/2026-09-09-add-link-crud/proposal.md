...
2. **Slug rules (D6)**: generated slugs 7 chars base62 from `random_int`, an insert-time collision retried with a fresh candidate on a reset EntityManager up to 5 times, then 500 with a logged error (design decision 3); custom slugs `^[A-Za-z0-9_-]{3,32}$` as the entire string, case-sensitive uniqueness (`abc` and `ABC` coexist), a single `ReservedSlugs` constant listing every top-level web/API route (`api`, `admin`, `login`, `logout`, `register`, `dashboard`, `links`, `api-keys`, `health`, `docs`, `qr`, `assets`, `build`, `bundles`, `_profiler`, `_wdt`, `_error`); the slug is immutable (a PATCH carrying a different slug → 422).
3. **Target URL policy (D8)** as a reusable validator (`src/Link/Validator/TargetUrl`): absolute URL, `http`/`https` only, host present, no userinfo, no backslash or control character anywhere, host not `localhost`, host mapped to ASCII per UTS #46 as browsers do, literal IPs — in every spelling a browser resolves to an address (shorthand, decimal, hex, octal, trailing dot, fullwidth digits), percent-encoded or unmappable hosts rejected — not in loopback, link-local or private ranges (v4 and v6), length ≤ 2048, no DNS resolution; the residual risk (public hostname resolving privately) is written into the how-to security notes. The same constraint will validate rule/variant targets in `add-routing-rules`.
