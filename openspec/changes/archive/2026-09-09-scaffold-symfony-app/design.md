...
3. **API Platform mounted at `/api/v1`, docs at `/api/docs`.** The spec fixes the behavior (`/api/v1` for operations, `/api/docs` and `/api/docs.json` for documentation); the configuration keys that produce it are discovered from the installed 4.3 package (`bin/console config:dump-reference api_platform`) during apply, not recalled from memory — the prefix handling changed across API Platform majors. Outcome: route prefix `/api` in `config/routes/api_platform.yaml`, `defaults.route_prefix: /v1` for operations, so `api_doc` stays at `/api/docs.{_format}`. The built-in entrypoint document exists only in JSON-LD/JSON:API, which the JSON-only format list excludes, so `enable_entrypoint: false`; the versioned base path `/api/v1` is a route to the documentation action instead (Swagger UI for browsers, the OpenAPI document for `Accept: application/json`). `mcp.enabled: false` (on by default in 4.3, not in scope). Formats: `json` only plus `jsonproblem`; `enable_swagger_ui: true`, `enable_re_doc: false`, `graphql` not installed. `info.title: Linkboard API`.
...
5. **Health controller in `src/Shared/Health/`**, plain Symfony controller outside API Platform (spec: not documented, no auth). Deep probe: raw `PDO` built from `DATABASE_URL` with `PDO::ATTR_TIMEOUT` (libpq connect_timeout; a DSN `connect_timeout` key is ignored by pdo_pgsql — measured 30 s vs 2 s) and a server-side `statement_timeout` of 2 s passed through libpq `options`, so a server that accepts the connection and then hangs is cut off too; `\Redis` from `REDIS_URL` with 2 s connect and read timeouts. Deliberately not the Doctrine connection or the cache pool: the probe must observe a down dependency without depending on services that fail to construct in that situation. `prod` → an explicit RFC 9457 `application/problem+json` 404 built by the controller itself (no framework renderer is guaranteed outside `/api`), replaced by a voter in change 3.
...
7. **Test harness.** PHPUnit 13 (`phpunit.dist.xml`, `KERNEL_CLASS`), `dama/doctrine-test-bundle` enabled as a PHPUnit extension and registered as a bundle for `test`; the test database is derived by Doctrine's `dbname_suffix: _test` (recipe default), so `.env.test` carries no `DATABASE_URL` of its own — the same URL serves both environments and CI. One new Makefile target `test-db` (`doctrine:database:create --if-not-exists --env=test` + `doctrine:migrations:migrate --env=test`), called by `make init` after `migrate`, so the first run leaves a usable test database; CI runs `make test-db EXEC=` before `make check EXEC=` against its service container. No throwaway sample tests: the health controller gets a Unit test of the probe result mapping, an Integration test of the deep probe against real postgres/redis, an Api test of `/health`, `/api/docs.json`, 404/405/406 problem details, and the 500 no-trace scenario through a `test`-only throwing route (a `when@test` block in `config/routes.yaml` → `tests/Api/Fixture/BoomController.php`; Symfony 8 has no per-environment routes directory by default). PHPUnit forces `APP_ENV=test` in both `$_SERVER` and `$_ENV`: `KernelTestCase` reads `$_ENV` first and the container's real environment carries `APP_ENV=dev`.
