...
- [ ] 2.3 `config/routes.yaml` declares `api_v1_graphql` at `/api/v1/graphql` on `api_platform.graphql.action.entrypoint` with **`methods: [POST]`**, mirroring the `api_v1_docs` route this repository already declares for the documentation controller (design decision 3a; Gate 1 round 1, finding 2 — the framework's own route is `/graphql` under the `/api` import prefix, so enabling the flag publishes `/api/graphql`, not the versioned path). Verify: `bin/console debug:router | rg graphql` lists both routes with their methods; a test posts a valid query to each and asserts the same data; and a test asserts `GET /api/v1/graphql` is 405.
- [ ] 2.5 What the framework's unversioned route answers to a `GET` is **measured, not assumed** (Gate 1 confirmation 1, finding 2): API Platform's route declares no method restriction and the package was not installed when these artifacts were written, so no status could be claimed for it. Verify: once the dependency exists, `curl -i` that path with `GET` and record the status and body shape here and in the how-to; if it turns out to serve anything a reader could mistake for an API response, say so plainly rather than leaving it undocumented.
...
- [ ] 4.3 The schema contains nothing else, and no mutation at all — and the check is automatic, which is what the living requirement now demands (Gate 1 confirmation 2, finding 1). Verify: a test introspects the shipped schema and asserts the set of query names **equals** a written-down list, so a new type fails it rather than joining it; that the mutation type is absent; that no type name derives from user administration, registration or API keys; **and** that every class carrying `#[ApiResource]` carries a `graphQlOperations` declaration, so a resource added without one fails here rather than reaching production with the framework's default mutations.
