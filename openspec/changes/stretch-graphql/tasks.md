...
- [x] 7.3 An internal failure leaks nothing outside `dev`. Verify, **measured in both environments**: in `dev` the depth refusal carries `extensions.file` naming a vendor file of the GraphQL library; in `prod` the same request answers `{"errors":[{"message":"Max query depth should be 10 but got 16."}]}` and nothing else. The test asserts the response carries no `/app/`, no `vendor/`, no `.php`, no `SELECT ` and no `App\\` — which is the promise, and which the `dev` behaviour is the documented exception to.
...
- [x] 8.2 `docs/reference/api-errors.md` carries the table of decision 5: which refusal is decided where, and therefore in which shape and with which status, plus the list of what "cannot be priced" means. Verify: the file re-read whole after the edit; every row of the table has a test behind it in section 7.
