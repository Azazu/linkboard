...
4. **Admin global statistics** (FR-ANL-4): `GET /api/v1/admin/stats/summary` (totals of users, links and clicks, clicks today), `/admin/stats/timeseries` (clicks and running total per bucket over all links, same parameters as the per-link one) and `/admin/stats/top-links` (top N links by clicks in the period with slug, owner id, clicks, unique visitors and rank) — admin only, same mechanisms, tag `global`. The global timeseries carries no distinct-visitor count: over every link's rows and every bucket it alone misses NFR-PERF-2 in every measured formulation (design appendix) — a revision of the original shape accepted by the user (User decisions). The admin summary has no period.
...
Recorded here because a `major` review finding is closed either by a fix or by the user's explicit acceptance (AGENTS.md, Review Protocol).

- **2026-09-11 — tier raised to `high`** (Gate 2 round 1, finding 1): Gate 1 on the artifacts in addition to Gate 2; rationale at the top of this file.
- **2026-09-11 — `top-links` keeps its unique visitors** (Gate 2 confirmation 1, finding 4): the original contract is met — the count is exact (the whole `visitor_hash`, compared under `COLLATE "C"`) and measured at 239 ms p95 uncached on one million clicks through the two-level statement of design decision 2. The first attempt (a 64-bit prefix of the hash, 260 ms) was rejected at Gate 2 confirmation 2 as lossy; the user chose the exact formulation over accepting an approximate count and authorised a third confirmation of round 1 (AGENTS.md: after two failed confirmations the user arbitrates).
- **2026-09-11 — the global `timeseries` carries no unique visitors** (Gate 2 confirmation 1, finding 4): accepted revision of the original "same shape as the per-link timeseries". Every measured formulation of the distinct count over every link's rows per bucket misses NFR-PERF-2 (534 ms as `count(DISTINCT visitor_hash)`, 356 ms at best — design appendix); the stretch `click_daily` aggregate is the way if it is ever wanted.
- **2026-09-11 — NFR-PERF-2's index clause reworded** (Gate 2 confirmation 1, finding 4): "every report query uses the `(link_id, occurred_at)` indexes" became "every per-link report query is bounded by the link's indexes — the planner picks the `(link_id)` index or the composite one depending on the window". The measurement shows the planner choosing the FK index `(link_id)` for a 30-of-60-day window with no loss (forcing the composite changes nothing), and the FK index cannot be dropped without fighting Doctrine (DBAL keeps an implicit FK index unless an existing index has exactly its columns — `Index::isFulfilledBy`). The 300 ms target is unchanged.
