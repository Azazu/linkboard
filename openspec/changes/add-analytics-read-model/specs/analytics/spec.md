...
- **THEN** the summary has `totalUsers` 3, `totalLinks` 3, `activeLinks` 2, `totalClicks` 12 (and answers the same with `?from=yesterday`, which it ignores), and the top links are A's 5-click link (rank 1) and B's 4-click link (rank 2), each with its `slug`, `ownerId` and `clicks` and without unique visitors
