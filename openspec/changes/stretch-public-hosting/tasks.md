...
- [ ] 2.1 Write the entrypoint (design decision 2): warm the cache, compile the asset map into the assets volume, and run `bin/console lexik:jwt:generate-keypair --skip-if-exists` into the keys volume, then exec the process the container is for. Verify, executed: a container starts and serves a request, and the entrypoint's output is recorded.
