...
- New dependencies (specification §5): `symfony/security-bundle` 8.1 (built in), `lexik/jwt-authentication-bundle` ^3.2 (JWT issuance/validation; Symfony has no issuer — §5 justification), `symfony/rate-limiter` 8.1 (built in), `symfony/form` + `symfony/validator` + `symfony/twig-bundle` (Twig already present; Form for the two web pages). CI: `make jwt-keys EXEC=` step; the spec table in §4 is unchanged.
