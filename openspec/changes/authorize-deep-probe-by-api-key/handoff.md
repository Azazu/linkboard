...
- Proposal (tier `high`), the health-check delta (admin key authorizes the `prod` probe; total budget 4 s enforced in code → 404 within 5 s in every failure mode; fail-closed identical 404; a Redis memory of the last verified key hash, TTL 300 s, consulted only when the database is unavailable, forgotten immediately on a negative answer — so the probe can report a database outage to a monitor verified before it), design (`Deadline` on `hrtime`, `AsyncPostgresQuery` on ext-pgsql's asynchronous API, `BoundedRedisCommands`, the CAS-ordered `ProbeMemory`, `ProbeAuthorizer` with the three-valued database step, fixtures for black-hole, delayed and response-losing connections, the applicability table), tasks (Gate 1, bounded clients, authorizer, prod end-to-end tests incl. the combined delayed-connect + stalled-statement case, CI migrating `app`, docs, wrap-up). `openspec validate --strict` and `scripts/pregate-verify.sh gate1` pass.
...
## Next step
`scripts/gate-run.sh authorize-deep-probe-by-api-key 1 confirm 1`; then `/opsx:apply authorize-deep-probe-by-api-key`.
