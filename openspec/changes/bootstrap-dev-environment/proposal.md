...
6. **Makefile guard (scope added 2026-09-08 after the Gate 2 floor run).** `make check` and its parts (`cs`, `stan`, `test`) SHALL exit 0 with an explicit `[SKIP] no composer.json — application not scaffolded yet` line while `composer.json` is absent, mirroring the CI `detect` job. Reason: the Gate 2 floor (`scripts/pregate-verify.sh gate2`) requires a green `make check`, checks are not waivable, and no application exists before change 2; without the guard this change cannot request Gate 2 at all. The guard is loud (prints SKIP, never silently passes) and disappears by construction the moment `composer.json` exists.
7. **Docs.** `docs/how-to/local-development.md` and `docs/reference/commands.md` describe the worker profile, AOF volume, the reset command covering both volumes, and state that `make init` becomes usable with change 2; `docs/explanation/requirements.md` §7 row 1: tier `high`, exit criterion restated as "`make up` brings postgres and redis healthy and nginx/php running; CI `workflow` job green, `php` job skipped by `detect`"; `openspec/ROADMAP.md` row 1 tier `high`.
...
- `docker-compose.yml` (worker service, redis command and volume, build args), `.env` (rename, build-arg defaults), `Makefile` (guard on `cs`/`stan`/`test` while `composer.json` is absent; `worker` target unchanged).
