...
9. **Makefile guard mirrors the CI `detect` job, as a Make-level conditional.** At parse time `ifeq ($(wildcard composer.json),)` sets `APP_MISSING := 1`; each of `cs`, `stan`, `test` wraps its recipe in `ifdef APP_MISSING … else … endif`, so the whole recipe is either the single `@echo "[SKIP] no composer.json — application not scaffolded yet"` line or the real `vendor/bin/*` invocation. A per-line shell guard with `exit 0` was rejected: Make runs each recipe line in its own shell, so the exit would return only from the guard line and the tool line would still run and fail. With the conditional, `make check` is green-with-SKIP on the empty app and becomes the real floor the moment change 2 adds `composer.json`. Alternatives rejected: skipping `make check` in `pregate-verify.sh` for `low`/docs changes (weakens the verifier for every future change); a waiver (AGENTS.md forbids waiving checks); holding this change until change 2 (leaves `main` red for another change). The guard cannot mask a real failure: with `composer.json` present it is inert.
