...
| Money rounding | n/a | |
| Empty/zero/null inputs | yes | `composer.json` absent → `detect` outputs `php=false` and the `php` job is skipped (the state of this repository until change 2); present → job runs. `.env` missing `HOST_UID`/`HOST_GID` → Compose falls back to `1000` via `${HOST_UID:-1000}`; a build with `HOST_UID=1234 HOST_GID=1234` is a required test (task 2.3) |
