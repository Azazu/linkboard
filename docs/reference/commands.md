...
| `make console ARGS='app:clicks:partitions'` | create every click partition the retention window and the horizon need; add `--retention` to also drop the months entirely outside the window (the only thing that deletes click data). Validates `CLICK_RETENTION_MONTHS` and `CLICK_PARTITION_HORIZON_MONTHS` before touching the schema |
