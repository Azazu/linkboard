...
|---|---|---|---|
| `/errors/400` | The body is not the JSON the operation accepts — malformed, or the wrong content type for the method. | Any operation with a request body; `PATCH` requires `application/merge-patch+json`. | Fix the payload or the `Content-Type`. Retrying the same request cannot succeed. |
...
| `/errors/403` | The credential is valid but may not do this: another user's resource, or an operation that needs `ROLE_ADMIN`; a blocked account is refused here too. | Every authenticated operation. | Do not retry with the same credential. |
...
| `/errors/406` | The `Accept` header admits none of the media types the operation produces. | Every operation. | Ask for `application/json` — or, for `GET /api/v1/links/{id}/qr`, `image/svg+xml` or `image/png`. |
