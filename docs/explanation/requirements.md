...
- **FR-KEY-1** A user SHALL create named API keys (`name` 1–64 chars after trimming, optional future `expires_at`), up to 10 active per user — the request that would create the 11th answers 409 problem details and creates nothing, also under concurrent creations (the count and the insert are serialised per owner in one transaction). The plaintext key is returned exactly once, on creation (`key` in the 201 body); the database stores `key_hash = SHA-256(key)` and a display `prefix` (first 8 characters). Keys have the form `lb_` + 40 base62 characters (≈238 bits of entropy).
- **FR-KEY-2** Authentication with an API key SHALL look the key up by the SHA-256 of the presented value (exact-match index lookup; no plaintext comparison exists), reject unknown, expired or revoked keys with the same 401 problem details (a blocked owner's key with 403 `blocked`), and update `last_used_at` at most once per minute per key (one conditional `UPDATE`, no ORM flush on the read path). A key never reaches the JWT authenticator and a JWT never reaches the key handler: the `lb_` prefix decides.
- **FR-KEY-3** A user SHALL list their keys (prefix, name, created, last used, expires, revoked — including revoked and expired ones, newest first) and revoke a key (`revoked_at` set; row retained for audit; idempotent 204). Keys are owner-only: another user's key — an admin's included — is a 404, indistinguishable from an unknown id.
- **FR-KEY-4** Rate limiting uses Symfony RateLimiter with Redis storage, sliding-window policy, limits from env with defaults: `RATE_LIMIT_REDIRECT_PER_IP=60` per minute, `RATE_LIMIT_API_PER_KEY=600` per minute (JWT-authenticated requests use the same limit keyed by user id), `RATE_LIMIT_AUTH_PER_IP=10` per minute. Over the limit → 429 problem details with `Retry-After`; `X-RateLimit-Limit` and `X-RateLimit-Remaining` headers are set on every response of a limited request — authenticated requests under `/api/v1` outside `/api/v1/auth/*`, counted once at authentication and before authorization, so refused (403/404) requests count too; anonymous and auth-endpoint responses carry none. The API limit fails open like the redirect limit: while its store is unreachable the request proceeds without headers and a `warning` is logged.
